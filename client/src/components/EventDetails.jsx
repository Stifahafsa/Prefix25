...
export default function EventDetails({ isOpen, onClose, eventId, onEdit }) {
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <h3 className="text-sm font-medium text-gray-500">ID de l'événement</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{formatDate(event.date_debut)}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500">Date de fin</h3>
                <p className="mt-1 text-sm text-gray-900">{formatDate(event.date_fin)}</p>
              </div>
            </div>

            <div>
              <h3 className="text-sm font-medium text-gray-500">Espace</h3>
              <p className="mt-1 text-sm text-gray-900">{event.espace_nom || `Espace #${event.espace_id}`}</p>
            </div>

            <div>
              <h3 className="text-sm font-medium text-gray-500">Prix</h3>
              <p className="mt-1 text-sm text-gray-900">
                {event.prix === 0 ? "Gratuit" : `${event.prix.toFixed(2)} MAD`}
              </p>
            </div>
            ...
            {event.description && (
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900 whitespace-pre-line">{event.description}</p>
              </div>
            )}
            ...
            {event.date_creation && (
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{formatDate(event.date_creation)}</p>
              </div>
            )}
          </div>
        ) : (
